...
*/
	...
	bug.chain = function( _key, _mode ){   // 1 start, 2 end
		...
		if( _mode == 1 ) {
			...
			item = this._chainLog[this._chainLog.length - 1];
			item.timeEnd = performance.now();
		} else {
			item = this._chainLog[this._chainLog.length - 1];
			item.timeEnd = performance.now();
	...
	bug.getChain = function() {
		return this._chainLog;
	}

	bug.chainShow = function() {
		var item;

		for(var i = 0; i < this._chainLog.length; i++) {
			item = this._chainLog[i];

			console.info(item.title, item.timeEnd - item.timeStart);
		}
	};
